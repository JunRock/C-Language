...
#include<stdlib.h>
void opening();
void card_binary(int num_count, unsigned int max_num, unsigned int* binary_num, unsigned int** num_card);
int main()
{
	unsigned int* binary_num; //2진수 숫자가 들어가는 배열
	unsigned int** num_card = { 0 };
	int i, j, k, t, count, r;
	int b = 65;
	int result = 1;
	int num_count;// 카드장수
	...
	scanf("%d", &num_count);
	binary_num = (int*)malloc(sizeof(int) * num_count);
	num_card = (int**)malloc(sizeof(int*) * num_count);
	system("cls");
	...
	{
		num_card[i] = (int*)malloc(sizeof(int) * max_num);
		for (j = 0; j < max_num; j++)
			num_card[i][j] = 0;
	}
	count = num_count * 6;
